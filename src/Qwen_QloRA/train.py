...
from Qwen_QloRA.constants import DEFAULT_MODEL_PATH, DEFAULT_OUTPUT_DIR, LABEL_COLUMNS
from Qwen_QloRA.data import DataCollatorForPreference, PreferenceDataset
from Qwen_QloRA.metrics import multiclass_log_loss, softmax
from Qwen_QloRA.modeling import load_qwen_sequence_classifier, load_tokenizer
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fine-tune Qwen2.5-3B with LoRA for A/B/tie classification."
    )
    parser.add_argument("--model", default=DEFAULT_MODEL_PATH, help="Local Qwen model path.")
    ...
    return apply_profile_defaults(parser.parse_args(), "qwen_train")
...
def main() -> None:
    ...
    predictions_path = output_dir / "qwen_qlora_valid_predictions.csv"
    target_modules = [
    ...
    print("[stage 3/5] Load Qwen classifier with LoRA")
    model = load_qwen_sequence_classifier(
    ...
    valid_loss = write_validation_predictions(trainer, valid_dataset, predictions_path)
    print(f"log_loss={valid_loss:.8f}")
    print(f"Wrote {predictions_path}")
    print(f"Wrote {output_dir / 'adapter'}")
