"""
Training script and model definition for the pairwise context-response encoder.

For each sample, the model builds two candidate sequences:
    prompt_a_context + response_a
    prompt_b_context + response_b
It encodes the two candidates with the same pretrained encoder and constructs
comparison features using the two vectors, their absolute difference, and their
elementwise product. The classifier outputs probabilities for A win, B win, and
Tie.

This file can be run directly to train the pairwise checkpoint and is also
...
def build_pair_text(prompt_context, response, side_name):
    prompt_context = "" if prompt_context is None else str(prompt_context)
    response = "" if response is None else str(response)
    return f"{prompt_context}\nCurrent {side_name} response:\n{response}"
...
class PairwiseTextDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            "pair_a": build_pair_text(row.prompt_a_context, row.response_a, "A"),
            "pair_b": build_pair_text(row.prompt_b_context, row.response_b, "B"),
...
class PairwiseCollator:
    ...
    def __call__(self, batch):
        return {
            "pair_a": self.tokenize_texts([item["pair_a"] for item in batch]),
            "pair_b": self.tokenize_texts([item["pair_b"] for item in batch]),
            "label": torch.tensor([item["label"] for item in batch], dtype=torch.long),
        }


class PreTokenizedPairwiseDataset(Dataset):
    def __init__(self, frame, tokenizer, max_length):
        pair_a = [build_pair_text(row.prompt_a_context, row.response_a, "A") for row in frame.itertuples(index=False)]
        pair_b = [build_pair_text(row.prompt_b_context, row.response_b, "B") for row in frame.itertuples(index=False)]
        self.pair_a = tokenizer(pair_a, padding="max_length", truncation=True, max_length=max_length, return_tensors="pt")
        self.pair_b = tokenizer(pair_b, padding="max_length", truncation=True, max_length=max_length, return_tensors="pt")
    ...
    def __getitem__(self, idx):
        return {
            "pair_a": {key: value[idx] for key, value in self.pair_a.items()},
            "pair_b": {key: value[idx] for key, value in self.pair_b.items()},
            "label": self.labels[idx],
        }


def stack_pairwise_batch(batch):
    def stack_encoded(name):
        return {key: torch.stack([item[name][key] for item in batch]) for key in batch[0][name]}

    return {
        "pair_a": stack_encoded("pair_a"),
        "pair_b": stack_encoded("pair_b"),
        "label": torch.stack([item["label"] for item in batch]),
    }


class PairwiseContextResponseEncoder(nn.Module):
    def __init__(
        self,
        model_name,
        projection_dim=512,
    ...
    def forward(self, pair_a, pair_b, return_logits=False):
        v_a = self.encode(pair_a)
        v_b = self.encode(pair_b)
        features = torch.cat([v_a, v_b, torch.abs(v_a - v_b), v_a * v_b], dim=-1)
        logits = self.classifier(features)
        return logits if return_logits else torch.softmax(logits, dim=-1)


def evaluate_pairwise(model, loader, criterion, device):
    model.eval()
    loss_sum = 0.0
    correct = 0
    seen = 0
    with torch.no_grad():
        for batch in loader:
            pair_a = move_encoded_to_device(batch["pair_a"], device)
            pair_b = move_encoded_to_device(batch["pair_b"], device)
            labels = batch["label"].to(device, non_blocking=True)
            logits = model(pair_a, pair_b, return_logits=True)
...
def main():
    ...
    parser.add_argument("--checkpoint-path", default=output_path("best_pairwise_context_response_encoder.pt"))
    parser.add_argument("--plot-path", default=output_path("pairwise_context_response_curves.png"))
    ...
    if args.no_pretokenize:
        ...
        collator = PairwiseCollator(tokenizer, max_length=args.max_length)
        train_dataset = PairwiseTextDataset(train_df)
        valid_dataset = PairwiseTextDataset(valid_df)
        collate_fn = collator
    else:
        print("Tokenization mode: pre-tokenize once and cache tensors")
        print("Pre-tokenizing train/valid pair text. This may take a moment, but training epochs will be faster.")
        train_dataset = PreTokenizedPairwiseDataset(train_df, tokenizer, args.max_length)
        valid_dataset = PreTokenizedPairwiseDataset(valid_df, tokenizer, args.max_length)
        collate_fn = stack_pairwise_batch
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        correct = 0
        seen = 0
        bar = tqdm(train_loader, desc=f"Pairwise Encoder Epoch {epoch}/{args.epochs}", dynamic_ncols=True)
        ...
        train_loss = loss_sum / seen
        train_acc = correct / seen
        valid_loss, valid_acc = evaluate_pairwise(model, valid_loader, criterion, device)
        history.append(
            {
                "epoch": epoch,
                "progress": total_seen / total_samples * 100 if total_samples else 0,
    ...
    model.eval()
    with torch.no_grad():
        sample_collator = PairwiseCollator(tokenizer, max_length=args.max_length)
        encoded = sample_collator(
            [
                {
                    "pair_a": build_pair_text(sample.prompt_a_context, sample.response_a, "A"),
                    "pair_b": build_pair_text(sample.prompt_b_context, sample.response_b, "B"),
                    "label": int(sample.class_label),
                }
            ]
        )
        probs = model(
            move_encoded_to_device(encoded["pair_a"], device),
            move_encoded_to_device(encoded["pair_b"], device),
        )[0].cpu()

    pred_label = int(probs.argmax().item())
    true_label = int(sample.class_label)
    print("\nFinal validation sample using best pairwise checkpoint:")
    print("id:", sample.id)
